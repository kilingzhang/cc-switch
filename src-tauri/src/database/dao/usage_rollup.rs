        ...
        match result {
            Ok(deleted) => {
                ...
                if deleted > 0 {
                    ...
                    // v12+: 明细归档为汇总后，触发跨设备用量自动上传
                    crate::services::usage_auto_sync::notify_rollup_done();
                }
                Ok(deleted)
            }
            Err(e) => {
