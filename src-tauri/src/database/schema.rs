...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    13 => {
                        log::info!("迁移数据库从 v13 到 v14（记录输入 token 缓存语义）");
                        Self::migrate_v13_to_v14(conn)?;
                        Self::set_user_version(conn, 14)?;
                    }
                    14 => {
                        log::info!("迁移数据库从 v14 到 v15（添加 Grok Build 代理配置）");
                        Self::migrate_v14_to_v15(conn)?;
                        Self::set_user_version(conn, 15)?;
                    }
                    15 => {
                        log::info!(
                            "迁移数据库从 v15 到 v16（Grok Build skill/mcp 启用状态改由本地 settings 管理）"
                        );
    ...
    fn migrate_v11_to_v12(conn: &Connection) -> Result<(), AppError> {
        ...
        if Self::table_exists(conn, "skills")?
            && (Self::has_column(conn, "skills", "enabled_claude")?
                ...
                || Self::has_column(conn, "skills", "enabled_grokbuild")?)
        {
            Self::rebuild_table_to_canonical(
                conn,
                "skills",
    ...
    fn export_activation_to_settings(conn: &Connection) -> Result<(), AppError> {
            ...
            && Self::has_column(conn, "mcp_servers", "enabled_claude")?
        {
            ...
            for (id, claude, codex, gemini, opencode, hermes) in rows {
                ...
                if flags.claude || flags.codex || flags.gemini || flags.grokbuild || flags.opencode || flags.hermes {
    ...
    /// v13 -> v14：记录 input_tokens 是否包含缓存写入。
    ///
    ...
    /// v14 -> v15: allow Grok Build to own an independent proxy configuration row.
    ...
    fn migrate_v15_to_v16(conn: &Connection) -> Result<(), AppError> {
        // feature/07 设备级设计：skill/mcp 的 per-app 启用状态在 settings.device_activation，
        // 不在 DB 增加 enabled_grokbuild 列。这里仅做兼容确认，保证从官方 v15 路径升级时
        // schema 版本号对齐到 16。
        let _ = conn;
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn migrate_v13_to_v14_adds_input_token_semantics_columns() -> Result<(), AppError> {
    ...
    #[test]
    fn migrate_v14_to_v15_adds_grokbuild_proxy_row_and_preserves_values() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
    ...
    #[test]
    fn migrate_v15_to_v16_keeps_skill_mcp_flags_in_settings() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
        conn.execute_batch(
            ...
            INSERT INTO mcp_servers (id, name, server_config) VALUES ('mcp-1', 'MCP', '{}');
            INSERT INTO skills (id, name, directory) VALUES ('skill-1', 'Skill', '/tmp/s');",
        )?;
        Database::set_user_version(&conn, 15)?;
        Database::apply_schema_migrations_on_conn(&conn)?;
        assert_eq!(Database::get_user_version(&conn)?, SCHEMA_VERSION);
        // feature/07: enabled_* 不落 DB，保持定义表干净
        assert!(!Database::has_column(&conn, "mcp_servers", "enabled_grokbuild")?);
